...
function ChargeableHoursCard({ allUsers, entries }) {
  // Chargeable = Normal Hours + Overtime + Block Course (not leave types)
  ...
  // Week picker state
  const [weekStart, setWeekStart] = useState(wsDate);

  // Compute week ending (Sunday)
  const weekEnd = (() => {
  ...
  // Chargeable entries: approved or sent to Xero (non-draft, non-declined)
  ...
  // Helper: apply per-apprentice overtime logic (same as Xero submission view)
  const calcOvertimeSplit = (app, appEntries) => {
    let normalHrs = 0, overtimeHrs = 0;
    const { overtimeType, overtimeThreshold } = app;
    const otConfigured = !!(overtimeType && overtimeThreshold);
    if (otConfigured) {
      const threshold = parseFloat(overtimeThreshold);
      if (overtimeType === "daily") {
        appEntries.forEach(e => {
        ...
        appEntries.forEach(e => {
          if (e.type === "Normal Hours") {
            const remaining = Math.max(0, threshold - normalHrs);
            normalHrs   += Math.min(e.netHours, remaining);
            overtimeHrs += Math.max(0, e.netHours - remaining);
          } else if (e.type === "Overtime") {
            overtimeHrs += e.netHours;
          } else {
            normalHrs += e.netHours;
    ...
    return { normalHrs, overtimeHrs, total: normalHrs + overtimeHrs, otConfigured };
  };

  // Group by host company → apprentice
  ...
  apprentices.forEach(app => {
    ...
    const { normalHrs, overtimeHrs, total, otConfigured } = calcOvertimeSplit(app, appEntries);
    const company = app.hostBusiness || "— No host business";
    if (!companies[company]) companies[company] = {};
    companies[company][app.name] = { normalHrs, overtimeHrs, total, otConfigured };
  });
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:13,color:T.sub}}>Approved entries — OT split per apprentice settings (⚠ = OT not configured in Users)</div>
          </div>
        </div>
        {/* Week navigator */}
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <button onClick={prevWeek} style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:7,
            padding:"5px 10px",cursor:"pointer",fontSize:14,fontFamily:"DM Sans,sans-serif",color:T.ink}}>‹</button>
          <div style={{fontSize:13,fontWeight:700,color:T.ink,minWidth:160,textAlign:"center"}}>
            {fmtD(weekStart)} – {fmtD(weekEnd)}
            {isCurrentWeek && <span style={{fontSize:11,color:T.teal,marginLeft:6,fontWeight:700}}>This week</span>}
          </div>
          <button onClick={nextWeek} disabled={isCurrentWeek}
            style={{background:T.bg,border:`1px solid ${T.border}`,borderRadius:7,
              padding:"5px 10px",cursor:isCurrentWeek?"not-allowed":"pointer",
              fontSize:14,fontFamily:"DM Sans,sans-serif",color:isCurrentWeek?T.muted:T.ink,
              opacity:isCurrentWeek?0.4:1}}>›</button>
        </div>
      </div>

      {companyKeys.length === 0 ? (
        <div style={{textAlign:"center",padding:"24px 0",color:T.muted,fontSize:14,fontStyle:"italic"}}>
          No Xero-submitted entries for this week
        </div>
      ) : (
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    ...
                    <div style={{display:"flex",gap:5,flexWrap:"wrap",justifyContent:"flex-end"}}>
                      ...
                      {!data.otConfigured && (
                        <span style={{fontSize:10,padding:"2px 7px",borderRadius:99,
                          background:T.warnL,color:T.warn,fontWeight:700}}
                          title="No overtime settings configured for this apprentice in Users">
                          ⚙ No OT config
                        </span>
                      )}
                    </div>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",alignItems:"center",gap:12,
            ...
            <span style={{fontSize:13,color:T.sub,fontWeight:700}}>Week total</span>
...
function ApprenticeSnapshotPanel({ apprenticeId }) {
  const [snapshots, setSnapshots] = useState([]);
  const [loading, setLoading] = useState(true);
      ...
      {panel && (CARDS.find(c=>c.id===panel)||{}).action === "panel" && (
        <div style={{
          ...
          <div style={{flex: 1, padding: "20px 20px 40px", maxWidth: 720, width: "100%", margin: "0 auto", boxSizing: "border-box"}}>
            ...
            {panel === "progress" && (
              <ApprenticeSnapshotPanel apprenticeId={currentUser.id}/>
            )}
